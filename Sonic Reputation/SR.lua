...
function fillReputation(i)
    ...
    if C_Reputation.IsMajorFaction(factionID) then
        ...
        barMin, barMax, barValue, level = 0, majorFactionData.renownLevelThreshold,
            majorFactionData.renownReputationEarned, majorFactionData.renownLevel;
    end
    local value = 0;
    ...
    elseif name and barMax then
        ...
        if (change > 0) then
            rep[name] = { barValue = barValue, lastBarMax = barMax }
            local msg = string.format(SR_REP_MSG, level and name .. "(" .. level .. ")" or name, barValue - barMin,
                barMax - barMin, change)
            createMessage(msg)
        end
    end
end
